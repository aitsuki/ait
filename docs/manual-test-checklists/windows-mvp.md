...
- [ ] 托盘图标常驻，右键菜单可用。
- [ ] 托盘右键菜单里的“显示翻译窗口”只显示窗口，不触发取词或翻译。
- [ ] 翻译窗口已显示时，托盘“显示翻译窗口”会将窗口带到最顶部并获取焦点。
- [ ] 设置窗口可保存默认提供方、OpenAI 配置、快捷键和剪贴板取词选项。
- [ ] API Key 保存后不明文出现在 `settings.json`。
- [ ] 翻译窗口可调整大小、可关闭，译文区可通过文本选区使用系统复制。
- [ ] 翻译窗口底部只保留“翻译”按钮。
- [ ] 翻译窗口按 `Esc` 可隐藏。
- [ ] 原文区和译文区支持双击、三连击、`Ctrl+A` 选区逻辑。
- [ ] 点击“翻译”只翻译原文区当前文本，不重新取词。
- [ ] 翻译窗口失焦后保持显示。

## 验证记录

- 2026-06-15：自动验证通过 `cargo fmt --check`、`cargo test`、`cargo build --release`。
- 2026-06-15：托盘、热键和窗口交互项需要在桌面会话中逐项人工勾选；本次未伪造勾选状态。
